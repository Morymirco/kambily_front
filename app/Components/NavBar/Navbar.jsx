...
export default function Navbar() {
	...
	// Provider de la carte
	const {cartItems, handleSetCartItems, loadCart, cartTotal, handleSetCartCount, addToCart, removeFromCart, updateQuantity, clearCart} = useCart();
	...
	return (
		<CartProvider>
			<nav className="w-full bg-white shadow-sm font-krub">
						...
						<div className="flex justify-between items-center text-sm">
							...
							<div className="flex items-center gap-1">
								...
								<div className="relative ml-6">
									...
									{showLanguages && (
										<div
											className="absolute top-full right-0 mt-2 w-40 bg-white rounded-lg shadow-lg z-50 py-2">
											{languages.map ((lang) => (
												<button
													key={lang.code}
													onClick={() => {
				...
				{/* Drawer Mobile */}
				<div
					className={`fixed inset-y-0 left-0 transform ${isDrawerOpen ? 'translate-x-0' : '-translate-x-full'} w-80 bg-white shadow-lg transition-transform duration-300 ease-in-out z-50`}>
						...
						<div className="flex-1 overflow-y-auto p-6">
							...
							<div className="mb-8">
								...
								<div className="relative group">
									...
									{/* Dropdown menu */}
									<div
										className="absolute left-0 mt-2 w-60 bg-white border rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
										{loadingCategories ? (
											// Skeleton loader
											...
											categories.map ((category) => (
												<div key={category.id || Math.random ()} className="relative group/sub">
													...
													{(category.subcategories?.length || 0) > 0 && (
														<div
															className="absolute left-full top-0 w-48 bg-white border rounded-lg shadow-lg opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-300">
															{category.subcategories.map ((sub) => (
																<Link
							...
							<div>
								...
								<div className="space-y-4">
									<div>
										<a href="tel:624-22-85-55"
										   className="flex items-center text-gray-800 hover:text-[#048B9A] gap-3">
											<div
									...
									<div>
										<a href="mailto:contact@kambily.com"
										   className="flex items-center text-gray-800 hover:text-[#048B9A] gap-3">
											<div
												className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center">
				...
				<div className="w-full">
					<div className="max-w-[1400px] mx-auto px-5 sm:px-8 md:px-12 lg:px-16 py-4 sm:py-4">
						<div className="flex items-center justify-between">
							...
							<div
								className="absolute left-1/2 transform -translate-x-1/2 sm:static sm:transform-none sm:ml-8">
							...
							<div className="flex items-center justify-end space-x-5 sm:space-x-6">
								...
								<Link href="/boutique"
								      className="hidden sm:flex items-center gap-2 text-gray-600 hover:text-[#048B9A]">
                ...
                > */}
									...
									/>
									<span
										className="absolute -top-2 -right-2 w-5 h-5 bg-[#048B9A] text-white text-xs rounded-full flex items-center justify-center">
                    {cartTotal}
                  </span>
									{/* </button> */}
                              ...
                              <span className="text-gray-600">
                                ...
                                {((cartTotal / 500000) * 100).toFixed (0)}%
															...
															<p className="text-sm text-[#048B9A] mt-2">
																Ajoutez {(500000 - cartTotal).toLocaleString ()} GNF au
																panier pour bénéficier de la livraison gratuite !
															</p>
														</>
													) : (
												...
												<div className="p-4 max-h-80 overflow-y-auto">
													{cartLoading ? (
														<div className="flex justify-center py-8">
															<div
																className="w-8 h-8 border-2 border-[#048B9A] border-t-transparent rounded-full animate-spin"/>
														</div>
													) : cartItems.length === 0 ? (
                            ...
                            {cartTotal.toLocaleString ()} GNF
								...
								>
									{isAuthenticated && user ? (
										<div
											className="relative w-8 h-8 rounded-full border-2 border-[#048B9A] overflow-hidden">
											{user.image ? (
												<Image
													src={user.image}
													alt="Profile"
												...
												<div
													className="w-full h-full bg-[#048B9A]/10 flex items-center justify-center">
                        <span className="text-[#048B9A] text-sm font-medium">
                          {user.first_name ? user.first_name[0].toUpperCase () : user.email[0].toUpperCase ()}
										...
										<div
											className="w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center hover:border-[#048B9A] transition-colors">
											<svg
												viewBox="0 0 24 24"
												className="w-4 h-4"
												fill="none"
