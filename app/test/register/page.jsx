...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    formDataToSend.append("phone_numer", formData.phone_number)
    ...
    // Inscription
    const registerResponse = axios.post('http://localhost:8001/accounts/register/', formDataToSend, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    registerResponse.then(response => {
      console.log('Réponse du serveur (inscription):', response.data);
      localStorage.setItem('access_token', response.data.access);
      localStorage.setItem('refresh_token', response.data.refresh);
      window.location.href = '/test/profile'; // a retravailler voir si on peut rediriger vers une page de confirmation de code
    })
