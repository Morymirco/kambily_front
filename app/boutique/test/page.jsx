...
import {HOST_IP, ITEMS_PER_PAGE, PORT, PROTOCOL_HTTP} from "../../constants";
...
export default function ProductList() {
	...
	const fetchProducts = useCallback(async (pageNumber = 1) => {
		try {
			...
			const response = await fetch(
				`${PROTOCOL_HTTP}://${HOST_IP}${PORT}/products/?page=${pageNumber}&limit=${ITEMS_PER_PAGE}`,
				{
					method: 'GET',
					headers: {
	...
	useEffect(() => {
		console.log("hello");
		fetchProducts(page).then(() => console.log("finished"));
	}, [page]); // ✅ Supprimer `fetchProducts` des dépendances
